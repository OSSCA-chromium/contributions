...
runtime enabled feature 플래그가 [C++에서 사용되는 경우](#1_in-c), `RuntimeEnabledFeatures::MyFeatureEnabled()`의 인수 없는 오버로드를 호출하는 모든 호출자를 `const FeatureContext*`를 받는 오버로드로 변경해야 한다. 여기에 `ExecutionContext`를 전달할 수 있다. 예를 들어 `ExecutionContext::From(ScriptState*)`를 사용한다.
...
또한 [제한 사항](#limitations) 때문에 runtime enabled feature 플래그는 브라우저 프로세스에서 **기본적으로** 사용할 수 없다.
...
[Blink Feature에서 `base::Feature` 인스턴스 생성]: /third_party/blink/renderer/platform/RuntimeEnabledFeatures.md#generate-a-instance-from-a-blink-feature
...
원문 링크: https://raw.githubusercontent.com/chromium/chromium/main/docs/origin_trials_integration.md

## History

- 2026-07-06: Chromium 원문 문서 `docs/origin_trials_integration.md` 전체 한국어 번역 노트 작성.

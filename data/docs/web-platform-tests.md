...
[web_tests/external/wpt](https://chromium.googlesource.com/chromium/src/+/main/third_party/blink/web_tests/external/wpt)에서 이루어진 변경 사항은 [자동으로 GitHub로 내보내집니다](#exporting-tests).
...
**참고**: `wpt_internal`의 큰 단점은 업스트림 리소스 스크립트(예: `testharness.js`) 변경으로 인해 테스트가 깨질 수 있다는 점입니다. `wpt_internal`은 다른 모든 비-`external/wpt` 테스트가 사용하는 fork된 `testharness.js` 버전을 사용하지 않기 때문입니다. 깨짐을 통지받을 수 있도록 [새 실패 알림](#new-failure-notifications)을 사용하는 것이 권장됩니다.
...
테스트를 리뷰할 때는 관련 명세와 일치하는지 확인하세요. 이는 구현과 완전히 일치하지 않을 수 있습니다. [명세에 맞춰 테스트 작성](#Write-tests-against-specifications)도 참조하세요.
...
가져오기가 GitHub의 인증되지 않은 요청 제한 때문에 실패할 수 있음을 기억하세요. 따라서 스크립트에 [GitHub 자격 증명을 전달](#GitHub-credentials)하는 것을 고려하세요.
...
원문: https://raw.githubusercontent.com/chromium/chromium/main/docs/testing/web_platform_tests.md

## History

- 2026-07-06: Chromium 원문 문서 `docs/testing/web_platform_tests.md`를 한국어 전체 번역 노트로 저장.

...
gn gen out/Default    # 빌드 디렉터리 생성
autoninja -C out/Default chrome   # 빌드
out/Default/chrome    # 실행 (Linux 기준)
```

빌드 시간을 줄이려면 `gn args out/Default`에서 다음 옵션을 고려하세요.
...
- 수정 후에는 관련 테스트를 함께 돌려보세요. 예:
  `autoninja -C out/Default blink_tests` 후 `run_web_tests.py` 실행.

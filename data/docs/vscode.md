...
*   **코드 편집**은 특히 [키보드 단축키](#키보드-단축키)에 익숙해지면 잘 동작합니다. VS Code는 반응성이 매우 좋고 Chromium 같은 큰 코드베이스도 처리할 수 있습니다.
...
VS Code에는 프로젝트 또는 솔루션 파일이 필요하지 않습니다. 하지만 기본 디렉터리(즉, 프로젝트 루트 폴더)의 `.vscode` 폴더에 워크스페이스 설정을 저장합니다. 자세한 내용은 [Chromium 워크스페이스 설정](#chromium용-설정) 섹션을 참조하세요.
...
*   [권장 확장 설치](#권장-확장-설치)의 지침을 따릅니다.
*   수동 설치. 확장 창(`Ctrl+Shift+X`, macOS에서는 `Cmd+Shift+X`)으로 이동해 다음 확장 이름을 검색합니다.

*** aside
참고: 아래에 언급된 모든 확장 설정은 이미 `tools/vscode/settings.json`에 설정되어 있습니다. 해당 파일을 워크스페이스로 복사하는 [지침](#chromium용-설정)을 따랐다면 아무것도 할 필요가 없습니다.
***
...
[lsp_patches_readme]: https://chromium.googlesource.com/chromium/src/+/main/tools/vscode/bazel_lsp/README.md
...
*   `F1` 또는 `Ctrl+Shift+P`는 명령(예: Tasks: Run Task)을 찾기 위한 검색 상자를 엽니다. 참고: [tasks.json의 사전 정의 태스크](#태스크) 중 하나를 실행하려면 `Ctrl+P` > "task <n>"을 사용하는 것이 더 빠릅니다.
...
[유용한 확장](#유용한-확장) 섹션에서 설명했듯이 Chromium 개발에 도움이 되는 필수 확장들이 있습니다. 아래 단계를 따르세요.
...
이제 `Ctrl+P`(macOS에서는 `Cmd+Shift+P`)를 사용하고 "task "와 원하는 숫자를 입력해 태스크를 실행할 수 있습니다. 빌드 태스크 중 하나를 선택하면 빌드 출력이 터미널 패널에 표시됩니다. `F8` / `Shift-F8`을 사용해 빌드 문제 사이를 빠르게 이동하세요. 태스크 실행에 대한 자세한 내용은 [태스크 이름](#태스크-이름)을 참조하세요.

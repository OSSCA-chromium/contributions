...
test('모임(meeting) 항목은 노란 모임 뱃지를 단다', () => {
  ...
  render(<ScheduleList meetings={[meeting]} />);
  // 목록 칩과 hover 팝오버 칩 양쪽 모두 노란(warning) 스타일이어야 한다.
  const chips = screen.getAllByText('모임');
  expect(chips.length).toBeGreaterThan(0);
  chips.forEach((chip) => expect(chip).toHaveClass('text-warning'));
});

test('제목이 미팅 상세 페이지로 링크된다', () => {
  render(<ScheduleList meetings={meetings} />);
  expect(screen.getByRole('link', { name: '발대식' })).toHaveAttribute(
    'href',
    '/schedule/2026-07-11-opening'
  );
});

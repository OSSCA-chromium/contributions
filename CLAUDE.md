...
Contribution frontmatter (see `data/contributions/template.md`): `title`, `date` (YYYY-MM-DD), `author` (GitHub username), `contribution_url`, `module` (single string — Chromium module/directory), `kind` (single string — change type, e.g. fix/feature/refactor/test/docs/cleanup; `module`/`kind` replace the old `labels` array), `status` (`merged` | `abandoned`; legacy files `6520751.md` and `8348446.md` still use `in review`). Optional fields: `repo` (only when not `chromium/src`, e.g. `devtools/devtools-frontend`), `issue` (this repo's assignment issue number), `crbug` (crbug ID from the commit message's `Bug:` footer), `related` (review ids of sibling patches). Copy `template.md` to `{ChromiumReviewId}.md` — the filename must equal the review id in `contribution_url`.
